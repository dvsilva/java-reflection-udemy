...
public class TestingFramework {
	...
	private Method findMethodByName(Method[] methods, String name) {
		for (Method method : methods) {
			if (method.getName().equals(name) && isParametrosERetornoValidos(method)) {
				return method;
			}
		}
	...
	private List<Method> findMethodsByPrefix(Method[] methods, String prefix) {
		...
		for (Method method : methods) {
			if (method.getName().startsWith(prefix) && isParametrosERetornoValidos(method)) {
	...
	private boolean isParametrosERetornoValidos(Method method) {
